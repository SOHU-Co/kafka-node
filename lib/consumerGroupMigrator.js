...
function ConsumerGroupMigrator (consumerGroup) {
  ...
  if (consumerGroup.options.migrateRolling) {
    ...
    this.zk.on('connected', function () {
      self.filterByExistingZkTopics(function (topics) { // no error in callback
        if (topics.length) {
          self.listenForChangesToTopics(topics);
        } else {
...
ConsumerGroupMigrator.prototype.filterByExistingZkTopics = function (callback) {
  ...
  async.filter(this.consumerGroup.topics, function (topic, callback) {
...
ConsumerGroupMigrator.prototype.listenForChangesToTopics = function (topics) {
  const self = this;
  const path = '/consumers/' + this.consumerGroup.options.groupId + '/owners/';
