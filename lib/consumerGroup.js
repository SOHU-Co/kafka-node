...
function ConsumerGroup (memberOptions, topics) {
  ...
  const self = this;
  this.options = _.defaultsDeep(memberOptions || {}, DEFAULTS);
        ...
        function (offsets, callback) {
          ...
          if (noOffset) {
            ...
            if (self.options.fromOffset === 'none') {
              return callback(
                new Error(
                  `${self.client.clientId} owns topics and partitions which contains no saved offsets for group '${self.options.groupId}'`
                )
              );
            }
