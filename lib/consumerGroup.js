...
ConsumerGroup.prototype.addTopics = function (topics, cb) {
  ...
  this.client.topicExists(topics, error => {
    if (error) {
      return cb(error);
    }
    ...
    async.waterfall([
      ...
      callback => {
        self.topics = self.topics.concat(topics);
        self.setupProtocols(self.options.protocol);
        self.connect.apply(self);
        callback(null);
      }
    ], error => {
      error ? cb(error) : cb(null, `Add Topics ${topics.join(',')} Successfully`);
    });
  });
};
