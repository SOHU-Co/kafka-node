...
function ConsumerGroup (memberOptions, topics) {
  ...
  if (this.options.heartbeatInterval < this.options.heartbeatTimeoutMs) {
    console.warn(`Heartbeat interval of ${this.options.heartbeatInterval} shouldn't be configured less than timeout ${this.options.heartbeatTimeoutMs}`);
    this.options.heartbeatInterval = this.options.heartbeatTimeoutMs - 1;
  }
...
ConsumerGroup.prototype.sendHeartbeat = function () {
  ...
  const heartbeat = new Heartbeat(this.client, this.options.heartbeatTimeoutMs, heartbeatCallback);
