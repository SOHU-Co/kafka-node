...
ConsumerGroup.prototype.resolveJoinGroup = function () {
  if (this.pendingJoinResolve) {
    this.pendingJoinResolve(true);
...
ConsumerGroup.prototype.connect = function () {
  ...
  async.waterfall(
    [
      function (callback) {
        if (typeof self.options.onRebalance === 'function') {
          logger.debug('Calling onRebalance');
      ...
      function (callback) {
        if (self.options.autoCommit && self.generationId != null && self.memberId) {
          logger.debug('Auto committing before rebalance');
      ...
      function (callback) {
        ...
        logger.debug('sendGroupCoordinatorRequest');
      ...
      function (coordinatorInfo, callback) {
        ...
        if (self.closing) {
          logger.debug('ConsumerClosing: GroupCoordinator');
          callback(null, null);
          return;
        }
        self.pendingJoin = new Promise((resolve, reject) => {
          self.pendingJoinResolve = resolve;
          logger.debug('sendJoinGroupRequest');
      ...
      function (joinGroupResponse, callback) {
        if (!joinGroupResponse && self.closing) {
          logger.debug('ConsumerClosing: GroupAssignment');
          callback(null, null);
          return;
        }
      ...
      function (groupAssignment, callback) {
        if (!groupAssignment && self.closing) {
          logger.debug('ConsumerClosing: GroupAssignment');
          callback(null, null);
          return;
        }
      ...
      function (syncGroupResponse, callback) {
        if (!syncGroupResponse && self.closing) {
          logger.debug('ConsumerClosing: SyncGroupResponse');
          callback(null, null);
          return;
        }
    ...
    function (error, startFetch) {
      ...
      self.resolveJoinGroup(); // Incase an error occurred or we shut down early
      if (self.closing) {
        logger.debug('ConsumerClosing: Leave Group');
        self.clearPendingFetches();
        self.leaveGroup((err) => {
          if (err) {
            logger.debug('Error leaving group after aborted connect', err);
          }
        });
        self.emit('connectaborted');
        return;
      }
      if (error) {
        logger.debug('Error during connection', error);
        self.recovery.tryToRecoverFrom(error, 'connect');
        return;
      }
...
ConsumerGroup.prototype.leaveGroup = function (callback) {
  ...
  var self = this;
  this.stopHeartbeats();
  if (self.generationId != null && self.memberId && !self.leavePending) {
    self.leavePending = true;
    async.waterfall([
      function (cb) {
        if (self.pendingJoin) {
          logger.debug('Waiting on a pending join to leave group');
          self.pendingJoin.then(() => cb(null));
        } else {
          cb(null);
        }
      },
      function () {
        ...
        logger.debug('sendLeaveGroupRequest');
        self.client.sendLeaveGroupRequest(self.options.groupId, self.memberId, function (error) {
          logger.debug('LeaveGroupRequest Done');
...
ConsumerGroup.prototype.close = function (force, cb) {
  ...
  async.series(
    [
      function (callback) {
        if (force) {
          logger.debug('ConsumerClosing: Committing');
      ...
      function (callback) {
        logger.debug('Leaving Group for close');
    ...
    function (error) {
      if (error) {
        // Commit failing is only way to end up here, caller may need to retry the close, or react to failed commit
        logger.debug('We had an error closing', error);
        self.emit('error', error);
        return cb(error);
      }
      logger.debug('ConsumerGroup Closed');
      self.closed = true;
      self.emit('close');
      cb(null);
    }
