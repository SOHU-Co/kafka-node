...
const protocol = require('./protocol_struct');
const getCodec = require('../codec');
const logger = require('../logging')('kafka-node:Protocol');
...
function decodeMessageSet (topic, partition, messageSet, cb, maxTickMessages, highWaterOffset) {
  ...
  while (messageSet.length > 0) {
    ...
    Binary.parse(messageSet)
      ...
      .tap(function (vars) {
        ...
        if (!vars.partial && vars.offset !== null) {
          ...
          // keep track of which order messages are in
          var offset = vars.offset;

          // * reserve offsets position "in reception order"
          messageOrderArray.push({ offset: offset, inlineMessageSet: null, error: null });

          codec.decode(vars.value, function (offset, error, inlineMessageSet) {
            var index = 0;
            var len = messageOrderArray.length;
            while (index < len) {
              if (messageOrderArray[index].offset !== offset) {
                ++index;
              } else if (messageOrderArray[index].offset === offset) {
                // index is where offset was stored in the ordering array
                messageOrderArray[index].inlineMessageSet = inlineMessageSet;
                messageOrderArray[index].error = error;

                // found offset at the top of the array? if so this is next message in order
                if (index === 0) {
                  // pop the top in order, while there are decoded results
                  while (index < len && messageOrderArray.length > 0 && (messageOrderArray[0].inlineMessageSet || messageOrderArray[0].error)) {
                    if (!messageOrderArray[0].error) {
                      decodeMessageSet(topic, partition, messageOrderArray[0].inlineMessageSet, cb, maxTickMessages);
                    } else {
                      // errors are logged in offset order too..
                      logger.error('Decompression error causing message loss on topic %s partition %s offset %s - error %s', topic, partition, messageOrderArray[0].offset, messageOrderArray[0].error);
                    }
                    messageOrderArray = messageOrderArray.shift();
                    ++index;
                  }
                }
                // all currently decoded is outputtet in order - so stop for now
                break;
              }
            }
          }.bind(null, offset));
    ...
    if (maxTickMessages && messageCount > maxTickMessages) {
      logger.warn('MaxTickMessages reached on topic %s partition %s', topic, partition);
      break;
    }
