...
function encodeOffsetFetchV1Request (group) {
  return function (clientId, correlationId, payloads) {
    return _encodeOffsetFetchRequest(clientId, correlationId, group, payloads, 1);
  };
}

function _encodeOffsetFetchRequest (clientId, correlationId, group, payloads, apiVersion) {
  payloads = groupByTopic(payloads);
  var request = encodeRequestHeader(clientId, correlationId, REQUEST_TYPE.offsetFetch, apiVersion);
  var topics = Object.keys(payloads);
...
function decodeGroupHeartbeat(resp) {
  ...
  Binary.parse(resp)
    ...
    .tap(function (vars) {
      if (vars.errorCode !== 0) {
        result = new Error(ERROR_CODE[vars.errorCode]);
        result.errorCode = vars.errorCode;
      }
    })
  return result;
}
...
function decodeSyncGroupResponse (resp) {
  ...
  var result = Binary.parse(resp)
    ...
    .tap(function (vars) {
      if (vars.errorCode !== 0) {
        error = new Error(ERROR_CODE[vars.errorCode]);
        error.errorCode = vars.errorCode;
      }
    })
    .word32bs('memberAssignment')
    .tap(function (vars) {
      this.buffer('memberAssignment', vars.memberAssignment);
      vars.memberAssignment = decodeMemberAssignment(vars.memberAssignment);
      delete vars.correlationId;
      delete vars.errorCode;
      delete vars.size
    })
    .vars;
...
function decodeMemberAssignment(assignmentBytes) {
  ...
  Binary.parse(assignmentBytes)
    ...
    .word32bs('userData')
    .tap(function (vars) {
      console.log('decodeMemberAssignment userData: ', vars.userData);
...
function _encodeMemberAssignment(assignment) {
  var assignmentByte = new Buffermaker()
    ...
    .Int32BE(assignment.topicPartitions.length);

  assignment.topicPartitions.forEach(function (topicPartition) {
    assignmentByte.Int16BE(topicPartition.topic.length).string(topicPartition.topic)
      .Int32BE(topicPartition.partition);
  })
