...
function _decodeFetchResponse(resp, cb) {
    ...
    for (var i=0; i< topicNum; i++) {
        ...
        for(var j=0; j<topic.partitionNum; j++) {
            ...
            var vars  = Binary.parse(resp)
                                ...
                                .vars;
            if (vars.errorCode !== 0) cb && cb(ERROR_CODE[vars.errorCode]);
            vars.messageSet = decodeMessageSet(topic.topic, vars.messageSet, cb);
    ...
    cb && cb(null, 'done', topics, responses);
    //return responses;
}

function decodeMessageSet(topic, messageSet, cb) {
    var set = [];
    while (messageSet.length > 0) {
        ...
        var message = decodeMessage(vars.message).value.toString();
        cb && cb(null, 'message', { topic: topic, message: message, offset: vars.offset });  
        set.push(vars);
    }
    return set;
}
