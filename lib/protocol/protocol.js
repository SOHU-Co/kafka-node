...
function messageMagic(apiVersion) {
  return apiVersion === 2 ? 1 : 0;
}
...
function encodeProduceRequest (requireAcks, ackTimeoutMs, apiVersion) {
  return function (clientId, correlationId, payloads) {
    return _encodeProduceRequest(clientId, correlationId, payloads, requireAcks, ackTimeoutMs, apiVersion);
  };
}
...
function decodeProduceResponse (resp) {
  ...
  Binary.parse(resp)
    ...
    .word32bs('topicNum')
    .loop(decodeTopics(decodePartitions))
    .word32bs('troh').tap(function (vars) {
  });
...
exports.decodeProduceV1ResponseV1 = decodeProduceV1Response;
...
exports.messageMagic = messageMagic;
