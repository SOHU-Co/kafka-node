...
const MAX_SUPPORT_VERSION = protocol.MAX_SUPPORT_VERSION;
...
function encodeOffsetCommitV2Request (clientId, correlationId, group, generationId, memberId, payloads) {
  encodeOffsetCommitGenericRequest(clientId, correlationId, group, generationId, memberId, payloads, 2);
}

function encodeOffsetCommitV1Request (clientId, correlationId, group, generationId, memberId, payloads) {
  encodeOffsetCommitGenericRequest(clientId, correlationId, group, generationId, memberId, payloads, 1);
}
  ...
  Binary.parse(resp)
    ...
    .loop(function (end, vars) {
      ...
      this.word16bs('apiKey')
        .tap(vars => apiKey = vars.apiKey)
        .word16bs('minVersion')
        .tap(vars => minVersion = vars.minVersion)
        .word16bs('maxVersion')
        .tap(vars => maxVersion = vars.maxVersion);
