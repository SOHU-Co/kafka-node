...
function _decodeFetchResponse(resp, cb) {
    ...
    for (var i=0; i< topicNum; i++) {
        ...
        for(var j=0; j<topic.partitionNum; j++) {
            ...
            vars.messageSet = decodeMessageSet(vars.messageSet, cb);
...
function decodeMessageSet(messageSet, cb) {
    var set = [];
    while (messageSet.length > 0) {
        ...
        var message = decodeMessage(vars.message);
        cb && cb(null, 'message', message.value.toString());  
        set.push(vars);
    }
    return set;
}
    ...
    for (var i=0; i<vars.topicNum; i++) {
        ...
        for (var j=0; j<topic.partitionNum; j++) {
            ...
            var p = Binary.parse(resp)
                ...
                .vars;
            resp = resp.slice(cur);
            topics[topic.topic][p.partition] = p.offset;
        } 
    }
console.log('hihi',topics);
    return topics;
}
