...
function decodeProduceResponse(resp) {
    ...
    function decodePartitions (end, vars) {
        if (--vars.partitionNum === 0) end();
        topics[vars.topic] = topics[vars.topic] || {};
        topics[vars.topic]['partitionNum'] = topics[vars.topic]['partitionNum'] || vars.partitionNum + 1;
        this.word32bs('partition')
            .word16bs('errorCode')
            .word64bs('offset')
            .tap(function (vars) {
                topics[vars.topic][vars.partition] = vars.offset;
                topics[vars.topic]['error'] = vars.errorCode;
                topics[vars.topic]['message'] = '';

                if (vars.errorCode !== 0) {
                    topics[vars.topic]['message'] = ERROR_CODE[vars.errorCode];
                }
            });
    }
    return topics;
