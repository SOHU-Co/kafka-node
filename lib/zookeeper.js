                ...
                if (error.name === 'NO_NODE') {
                    return that.client.mkdirp(path, function (error, path) {
                        if (error && error.name !== 'NODE_EXISTS') {
                            console.warn('Failed to create empty broker path %s due to: %s.', path, error);
                            setTimeout(relist, 100);
                            return;
                        }
                        relist();
                    });
                }
                return console.warn('Failed to list children of node: %s due to: %s.', path, error);
            }
