...
Zookeeper.prototype.getConsumersPerTopic = function (groupId, cb) {
    ...
    var path = '/consumers/' + groupId;
    var that = this;
    var consumerPerTopicMap = {'consumerTopicMap': [], 'topicConsumerMap': [], 'topicPartitionMap': []};
    ...
    async.series([
                                    ...
                                    try {
                                        ...
                                        for (var topic in obj.subscription) {
                                            if (consumerPerTopicMap['topicConsumerMap'][topic] == undefined) {
                                                consumerPerTopicMap['topicConsumerMap'].push(topic);
                                            ...
                                            if (consumerPerTopicMap['consumerTopicMap'][consumer] == undefined) {
                                                consumerPerTopicMap['consumerTopicMap'].push(consumer);
            ...
            function (callback) {
                consumerPerTopicMap['topicConsumerMap'].map(function (topic) {
                    consumerPerTopicMap['topicConsumerMap'][topic] = consumerPerTopicMap['topicConsumerMap'][topic].sort();
                    return;
                });
                callback();
            },
            function (callback) {
                async.each(consumerPerTopicMap['topicConsumerMap'], function (topic, cbb) {
                    ...
                    that.client.getData(
                        ...
                        function (error, data) {
                            if (error) {
                                ...
                                if (consumerPerTopicMap['topicPartitionMap'][topic] == undefined) {
                                    consumerPerTopicMap['topicPartitionMap'].push(topic);
                                    consumerPerTopicMap['topicPartitionMap'][topic] = [];
                                }
                                consumerPerTopicMap['topicPartitionMap'][topic].push(partitions.sort(compareNumbers));
                                cbb();
                            }
                        }
                    );
