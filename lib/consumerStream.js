...
/**
 * Either emits the message or buffers it depending on the downstream streams.
 */
...
ConsumerStream.prototype.transmitMessages = function() {
    ...
    while (keepGoing && this.messageBuffer.length > 0) {
        keepGoing = this.push(this.messageBuffer.shift());
    }
    if (this.messageBuffer.length == 0 && keepGoing) {
        this.fetch();
    }
}
    ...
    if (!self.fetchInFlight) {
        ...
        self.client.send(self.payloads, encoder, decoder, function (err) {
            if (err) {
                ...
                self.emit.apply(consumer, arguments);
            }
        });
    }
};
