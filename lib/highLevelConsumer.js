...
var DEFAULTS = {
    ...
    fromOffset: false,
    rebalanceRetries: 10,
    rebalanceMinTimeout: 100,
    rebalanceMaxTimeout: 1000,
    partitionOwnershipInterval: 20000
};
...
HighLevelConsumer.prototype.connect = function () {
    ...
    }, this.options.partitionOwnershipInterval);
    ...
    function rebalance() {
        ...
        if (!self.rebalancing) {
            ...
            var operation = retry.operation({
                retries: self.options.rebalanceRetries,
                factor: 2,
                minTimeout: self.options.rebalanceMinTimeout,
                maxTimeout: self.options.rebalanceMaxTimeout,
                randomize: true
            });
