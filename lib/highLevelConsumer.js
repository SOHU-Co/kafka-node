...
var HighLevelConsumer = function (client, topics, options) {
    ...
    this.empty_payloads = this.buildEmptyPayloads(topics);
    this.connect();
}
...
HighLevelConsumer.prototype.buildEmptyPayloads = function (empty_payloads) {
    var self = this;
    return empty_payloads.map(function (j) {
...
HighLevelConsumer.prototype.connect = function () {
    ...
    // this is emitted when there is a problem fetching from the partitions offset - request offsets
    this.on('offsetOutOfRange', function (err) {
        var request = [];
        self.topicPayloads.forEach(function (p) {
            request.push({'topic': p.topic, 'partition': p.partition || 0, 'maxNum': self.options.maxNumSegments, 'metadata': 'm', 'time': Date.now()});
        });
        self.offsetRequest(request, function (err, offsets) {
            if (err) elf.emit('error', new errors.InvalidConsumerOffsetError(self));
            else {
                self.topicPayloads.forEach(function (t) {
                    if (t.offset < offsets[t.topic][t.partition][offsets[t.topic][t.partition].length-1])
                        t.offset = offsets[t.topic][t.partition][offsets[t.topic][t.partition].length-1];
                });
            }
        });
...
HighLevelConsumer.prototype.fetch = function () {
    ...
    if (this.paused) {
        this.client.sendFetchRequest(this, this.empty_payloads, this.options.fetchMaxWaitMs, 0);
    }
    else {
