...
HighLevelConsumer.prototype.addTopics = function (topics, cb) {
  ...
  this.client.addTopics(
    ...
    function (err, added) {
      ...
      var payloads = self.buildPayloads(added);
      var offsetPayloads = self.buildOffsetPayloads(added);
      // update offset of topics that will be added
      self.fetchOffset(offsetPayloads, function (err, offsets) {
        if (err) return cb(err);
...
HighLevelConsumer.prototype.buildOffsetPayloads = function (payloads) {
  return _.reduce(payloads, function (cumu, p) {
    var topic;
    if (typeof p !== 'object') {
      topic = p;
      p = { topic: topic };
