...
HighLevelConsumer.prototype.rebalanceAttempt = function (oldTopicPayloads, cb) {
    ...
    console.log("HighLevelConsumer %s is attempting to rebalancezzzzzzz", self.id);
    async.series([
            ...
            function (callback) {
                ...
                for (var topic in consumerPerTopicMap['consumerTopicMap'][self.id]) {
                    ...
                    var numberOfPartition = consumerPerTopicMap['topicPartitionMap'][topicToAdd][0].length;
                    ...
                    for (var i = startPart; i < startPart + nParts; i++) {
                        newTopicPayloads.push({ topic: topicToAdd, partition: consumerPerTopicMap['topicPartitionMap'][topicToAdd][0][i], offset: -1, maxBytes: self.options.fetchMaxBytes, metadata: 'm'});
                    }
                }
                callback();
            },
