 ...
 * autoCommit is discouraged for asynchronous code, since it can commit unprocessed messages
 */
...
MessageConsumer.prototype.onBeforeFetch = function (cb) { // XXX not wait-free
    console.log('======== waiting for before fetch');
    if (this.fetching) {
...
MessageConsumer.prototype.sendCommit = function (commit, failure /* (failedPayload) */, cb /* (err) */) {
    var self = this;
    var tasks = [];
    ...
    if (self.options.useCommitApi) { // XXX doesn't normalize results
...
MessageConsumer.prototype.syncedRebalance = function (cb) { // XXX sync
    var self = this;
    if (this.closing) {
        console.log('closing, not rebalancing');
        return;
    }
...
MessageConsumer.prototype.glob = function (path, transform /* (path, child) optional */, cb) {
    ...
    this.getChildren(path, function(err, children, stat) {
        console.log('======= glob getChildren', err, children);
        if (err) {
            return cb(err);
        }
...
MessageConsumer.prototype.rebalance = function (cb /* (err) */) {
    ...
    async.parallel([
        ...
        console.log('trying to rebalance', err, vals);
        if (err) {
            cb(err);
        }
        ...
        function potluck (cb) { // never self.consumer.payloads.push(newPayload)
            async.series([
                ...
                function (cb) {
                    console.log('giving ' + give.length + ' partition(s)');
                    if (!give.length) {
                        return cb();
                    }
                ...
                function (cb) { // commit ownership
                    console.log('taking ' + take.length + ' partition(s)');
                    if (!take.length) {
                        return cb();
                    }
                    concurrent.parallelLocked(take.map(function (payload) {
                        ...
                        return function (cb) {
                            concurrent.retry(self.options.rebalanceMaxAttempts, thunk, cb); // XXX let rebalance interrupt this
                        };
                    }), cb);
                },
                function (cb) { // get offsets
                    ...
                    console.log('loading ' + payloads.length + ' partition(s)');
                    if (!payloads.length) {
                        return cb();
                    }
                ...
                function (cb) {
                    console.log('got offsets', { newPayloads: newPayloads, payloads: self.consumer.payloads }); // ===========
                ...
                function (cb) {
                    ...
                    self.consumer.ready = true; // XXX wait?
                    self.fetching = true;
                    self.consumer.fetch();
                    cb();
                },
...
MessageConsumer.prototype.close = function (cb) {
    ...
    async.series([
        self.onBeforeFetch.bind(self),
        function (cb) {
            self.fetching = self.ready = false;
            cb();
        },
