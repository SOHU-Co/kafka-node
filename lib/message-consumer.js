...
MessageConsumer.prototype.getOffset = function (topic, partition, cb) {
    return this.getData('/consumers/' + this.consumer.options.groupId + '/offsets/' + topic + '/' + partition, cb);
};

MessageConsumer.prototype.setOffset = function (topic, partition, offset, cb) {
    return this.setData('/consumers/' + this.consumer.options.groupId + '/offsets/' + topic + '/' + partition, offset, cb);
};

MessageConsumer.prototype.givePartition = function (topic, partition, cb) {
    return this.zk().delete('/consumers/perf-consumer/owners/test/1', cb);
};

MessageConsumer.prototype.takePartition = function (topic, partition, cb) {
    return this.create('/consumers/' + this.consumer.options.groupId + '/owners/' + topic + '/' + partition, new Buffer(this.consumerId), zookeeper.CreateMode.EPHEMERAL, cb);
};
    ...
    async.parallel([
        ...
        async.series([
            function (cb) {
                ...
                if (self.started) {
                    self.consumer.on('done', commitGifts);
                } else {
                    commitGifts();
                }
                self.started = true;
                function commitGifts() {
                    console.log('committing gifts');
                    var commits = [], payloads = [];
                    ...
                    var tasks = commits.map(function (payload) {
                        return self.thunk('setOffset', payload.topic, payload.partition, payload.offset);
                    });
                    tasks = []; // XXX don't use ZK
                    if (commits.length) {
                        tasks.push(function (cb) {
                            self.consumer.client.sendOffsetCommitRequest(self.consumer.options.groupId, commits, cb);
                            self.consumer.payloads = payloads;
                            cb();
                        });
                    }
            ...
            function (cb) { // commit ownership
                ...
                async.parallel(take.map(function (payload) {
                    return self.thunk('takePartition', payload.topic, payload.partition);
                }), cb);
            },
