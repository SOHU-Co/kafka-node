...
MessageConsumer.prototype._invalidateBalance = function () { // TODO add delay
    this.syncedRebalance(function (err) {
        if (err) {
...
MessageConsumer.prototype.syncedRebalance = function (cb) { // XXX sync
    async.retry(this.options.rebalanceMaxRetries, this.rebalance.bind(this), cb);
};
...
MessageConsumer.prototype.rebalance = function (cb /* (err) */) {
    ...
    async.parallel([
        ...
        topics.forEach(function (topic) {
            ...
            console.assert(myConsumerPosition >= 0, 'cannot find ' + consumerThreadId + ' in ' + curConsumers);
