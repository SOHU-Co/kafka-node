            ...
            async.series([
                        ...
                        if (self.options.useCommitApi) {
                            if (give.length) {
                                tasks.push(function (cb) {
                                    self.consumer.client.sendOffsetCommitRequest(self.consumer.options.groupId, give, function (err) {
                                        if (err) {
                            ...
                            tasks = give.map(function (payload) {
                                return function (cb) {
                    ...
                    if (self.options.useCommitApi) {
                        self.consumer.fetchOffset(take, function (err, offsets) {
                            ...
                            take.forEach(function (p) {
                                p.offset = offsets[p.topic][p.partition]; // XXX commits -1
                            });
                            cb();
                        });
                    } else {
                            ...
                            return function (cb) {
                                self.getOffset(payload.topic, payload.partition, function (err, offsetString) {
                                    ...
                                    if (isNaN(payload.offset)) {
                                        payload.offset = -1; // XXX commits -1
                                    }
                ...
                function (cb) {
                    ...
                    self.consumer.client.sendOffsetRequest(virginPayloads, function (err, offsets) {
                        ...
                        console.log('sendOffsetRequest', offsets);
                        cb();
                    });
                },
                function (cb) {
