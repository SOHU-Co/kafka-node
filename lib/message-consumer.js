...
MessageConsumer.prototype._registerConsumer = function (cb /* (err) */) {
    var self = this;
    async.parallel([
        this.thunk('getChildren', '/consumers/' + this.consumer.options.groupId + '/ids', function (event) {
            console.log('consumer registry event', event);
            if (event.name !== 'NODE_CHILDREN_CHANGED') {
                console.warn('unknown consumer registry event', event);
            }
