...
Consumer.prototype.fetchOffset = function (payloads, cb) {
    this.client.sendOffsetFetchRequest(this.groupId, payloads, cb);
}
...
Consumer.prototype.addTopics = function (topics, cb) {
    ...
    this.client.addTopics(
        ...
        function (err, added) {
            ...
            var offsets = {};
            self.fetchOffset(payloads, function (err, data) {
                if (err) return cb(err);
                offsets = _.extend(offsets,data);
                // Offset fetching
                if (Object.keys(offsets).length < payloads.length) return;
                payloads.forEach(function (p) { 
