...
var Consumer = function (client, topics, groupId, fetchMaxWaitMs, fetchMinBytes) {
    if (!topics) throw new Error('Must have payloads');
    this.client = client;
    this.groupId = groupId || 'kafka-node-group';
    this.fetchMaxWaitMs = fetchMaxWaitMs || DEFAULTS.fetchMaxWaitMs;
    this.fetchMinBytes = fetchMinBytes || DEFAULTS.fetchMinBytes;
    this.ready = false;
    this.offsets = {};
...
Consumer.prototype.init = function () {
    this.fetchOffset(this.payloads, function (err, topics) {
        ...
        this.updateOffsets(topics, true);
        this.fetch();
    }.bind(this));
}
/* 
...
Consumer.prototype.fetch = function () {
    ...
    this.client.sendFetchRequest(this, payloads, this.fetchMaxWaitMs, this.fetchMinBytes);
}
