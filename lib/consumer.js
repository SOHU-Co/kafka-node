...
Consumer.prototype.ensureFetching = function () {
    if (!this.fetching) {
        this.fetch();
    }
}
...
Consumer.prototype.fetch = function () {
    ...
    var payloads = this.payloads.filter(function (payload) {
        return payload.offset >= 0 && !payload.busy;
    });
    if (payloads.length) { // empty fetch confuses kafka
        this.fetching = true;
        console.log('=========== fetching', payloads.length);
