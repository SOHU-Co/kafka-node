...
Consumer.prototype.connect = function () {
  ...
  this.on('offsetOutOfRange', err => {
    self.pause();
    const offsetObj = new Offset(self.client);
    // we can ignore this since we are already forwarding error event emitted from client
    offsetObj.on('error', _.noop);
    offsetObj.fetch(self.payloads, (err, serverOffsets) => {
      if (err) {
        self.emit('error', err);
        return;
      }
      self.payloads.forEach(function (payload) {
        let offset = _.head(serverOffsets[payload.topic][payload.partition]);
        if (offset === -1) offset = 0;
        payload.offset = offset;
        logger.debug(`Fixing offset for topic '${payload.topic}' partition '${payload.partition}' to ${offset}`);
      });
      self.resume();
    });
  });
