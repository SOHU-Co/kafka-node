...
KafkaClient.prototype.brokerForLeader = function (leader, longpolling) {
  ...
  // If leader is not give, choose the first connected broker as leader
  if (typeof leader === 'undefined') {
    connectedBrokers = Object.keys(brokers).filter(k => brokers[k].isConnected());
    if (connectedBrokers.length > 0) {
      addr = connectedBrokers[0];
      logger.debug(`${this.clientId} brokerForLeader - brokers addr exist::  ${addr}`);
      return brokers[addr];
    } else if (!_.isEmpty(this.brokerMetadata)) {
      leader = Object.keys(this.brokerMetadata)[0];
      logger.debug(`${this.clientId} brokerForLeader - leader:: ${leader}`);
    } else {
      logger.debug(`${this.clientId} brokerForLeader - leader else return`);
      return;
    }
  }
  ...
  if (!broker) {
    logger.debug(`${this.clientId} brokerForLeader - NO broker!!`);
    return;
  }
  ...
  return (
    ...
    this.setupBroker(broker.host, broker.port, longpolling, brokers, err => {
      if (err) {
        logger.debug(`${this.clientId} setupBroker error!!`);
        this.emit('error', err);
      }
    })
  );
