...
KafkaClient.prototype.connectToBroker = function (broker, callback) {
  ...
  const onError = (error) => {
    ...
    socket.closing = true;
    socket.end();
    delete this.brokers[`${broker.host}:${broker.port}`];
    callback(error);
  };
...
KafkaClient.prototype.getAvailableBroker = function (callback) {
  ...
  this.connectToBrokers(brokersToTry, function (error, broker) {
    if (error) {
...
KafkaClient.prototype.updateMetadatas = function (metadatas) {
  ...
  Client.prototype.updateMetadatas.call(this, metadatas);
};
