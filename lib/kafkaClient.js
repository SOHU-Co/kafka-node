  ...
  async.doWhilst(
    callback => {
      this.connectToBroker(hosts[index++], (error, connectedBroker) => {
        ...
        errors.length = 0;
        this._connectedBroker = connectedBroker;
        callback(null);
      });
    },
    () => !this.closing && !this._connectedBroker && index < hosts.length,
    () => {
      if (this.closing) {
        return callback(new Error('client is closing'));
      }

      if (this._connectedBroker) {
        return callback(null, this._connectedBroker);
      }
