...
Producer.prototype.send = function (payloads, cb) {
    ...
    async.waterfall([
        ...
        function (callback) {
            ...
            if (self.options.metadataRefreshIntervalMs >= 0 && now - self.lastMetadataRefreshTime > self.options.metadataRefreshIntervalMs) {
                self.lastMetadataRefreshTime = now;
        ...
        function (callback) {
            self.buildPayloads(payloads, function (err, produceRequests) {
            if (err) {
                callback(err);
            } else {
                ...
                self.client.sendProduceRequest(groupedProduceRequests, self.options.requireAcks, self.options.ackTimeoutMs, callback);
            }
        })
    }],
    function (err, result) {
...
Producer.prototype.createTopics = function (topics, async, cb) {
    ...
    this.client.loadMetadataForTopics(topics, function (err, resp) {
        if (err) {
            ...
            // If error is LeaderNotAvailable, disregard it since we always get this error when loading metadata for non existing topics.
            if (err instanceof liberrors.TopicsError) {
                var errorCodes = _.chain(err.topics)
                                  .values()
                                  .toArray()
                                  .groupBy(function (val) { return val.errorCode; })
                                  .keys()
                                  .value();

                if (errorCodes.length === 1 && errorCodes[0] == 5) {
                    throwError = false;
                }
            }
