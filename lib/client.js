...
Client.prototype.updateMetadatas = function (metadatas) {
    ...
    _.extend(this.topicMetadata, metadatas[1]);
    ...
    if (payloads[1].length) {
        ...
        this.loadMetadataForTopics(topicNames, function (err, resp) {
            ...
            payloads[1].forEach(function (p) {
                if (!self.hasMetadata(p.topic, p.partition, metadata)) error = p;
            });
            if (error) 
                return cb('No metadata for [topic: ' + error.topic + ', partiiton: ' + error.partition + ']');
...
Client.prototype.sendFetchRequest = function (consumer, payloads, fetchMaxWaitMs, fetchMinBytes) {
    ...
    this.sendToBroker(payloads, encoder, decoder, 'message'); 
}
var c;
...
//if (global.s<35000) console.log('s:',global.s,'r:', global.r, 'm:', global.m, 'd:', global.d)
        ...
        var error = reply ? reply.error : null;
        if (typeof cb === 'function')
            error ? cb.call(this, error) : cb.call(this, null, reply);
