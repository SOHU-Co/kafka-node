...
Client.prototype.close = function (cb) {
  ...
  this.closing = true;
  if (Object.keys(this.cbqueue).length || this.closed) {
  ...
  this.zk.once('close', () => this.emit('close'));
  this.zk.close();
};

Client.prototype.closeBrokers = function (brokers) {
  logger.debug(`closing brokers`, Object.keys(brokers));
...
Client.prototype.refreshMetadata = function (topicNames, cb) {
  ...
  function attemptRequestMetadata (topics, cb) {
    ...
    const closingError = new errors.ClientIsClosing(); // Generate stack trace for caller if used while closing
...
Client.prototype.createBroker = function (host, port, longpolling) {
  ...
  function retry (s) {
    if (s.retrying || s.closing || self.closing) return;
    s.retrying = true;
