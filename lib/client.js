...
Client.prototype.setupBrokerProfiles = function (brokers) {
  ...
  Object.keys(brokers).forEach(function (key, index) {
...
Client.prototype.reconnectBroker = function (oldBroker) {
  ...
  var brokers = this.getBrokers(oldBroker.longpolling);
  var brokerKey = oldBroker.addr;
  if (!(brokerKey in brokers)) {
    brokerKey = _.findKey(brokers, {socket: oldBroker});
  }
  var newSocket = this.setupBroker(oldBroker.host, oldBroker.port, oldBroker.longpolling, brokers, brokerKey);
