...
function splitOnFirst(string, delimiter) {
    var index = string.indexOf(delimiter);
    if(~index) {
        return [string.substring(0, index), string.substring(index + delimiter.length)];
    } else {
        return [string];
    }
}
        ...
        Object
            ...
            .forEach(function (key, index) {
                ...
                if (self.ssl) {
                    ...
                    sslAddress = splitOnFirst(sslAddress, '://')[1];
                    var parts = splitOnFirst(sslAddress, ':');

                    brokerProfile.sslHost = parts[0];
                    brokerProfile.sslPort = +parts[1];
                }

                // Only connect one broker
                if(index === 0) {
                    self.brokers[addr] = self.createBroker(brokerProfile.host, brokerProfile.port);
                }
            });
        self.emit('ready');
    });
