...
/* Helper method which groups messages by (topic, partition) */
function groupByTopicPartition(payloads) {
    var p;
    var i = 0;
    var temp = {};
    var out = [];
    ...
    while ((p = payloads[i++])) {
        ...
        if (!temp.hasOwnProperty(key)) {
            ...
            var messages = temp[key].messages;
            messages.push.apply(messages, p.messages);
        }
    }
...
Client.prototype.sendProduceRequest = function (payloads, requireAcks, ackTimeoutMs, cb) {
    payloads = groupByTopicPartition(payloads);
...
Client.prototype.setupBroker = function (host, port, longpolling, brokers) {
    var addr = host + ':' + port;
    brokers[addr] = this.createBroker(host, port, longpolling);
    return brokers[addr];
};
...
Client.prototype.reconnectBroker = function (oldBroker) {
    oldBroker.retrying = false;
    if (oldBroker.error) {
        oldBroker.destroy();
    }
    var newSocket = this.setupBroker(oldBroker.host, oldBroker.port, oldBroker.longpolling, this.getBrokers(oldBroker.longpolling));
    newSocket.error = oldBroker.error;
};
