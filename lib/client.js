...
var net = require('net'),
    ...
    SmartBuffer = require('./socketBuffer/smartBuffer.js').SmartBuffer,
    argv = require('optimist').argv;

var verbose = argv.verbose && argv.verbose == "true";
if (verbose) console.log("*** CLIENT IS VERBOSE ***");
...
Client.prototype.createBroker = function connect(host, port, longpolling) {
    ...
    socket.buffer = new SmartBuffer(10 * 1024 * 1024, 0.2);

    socket.on('data', function (data) {
        if (verbose) console.log("received data", data.length);
        this.buffer.write(data);
        if (verbose) console.log("buffer length after write", this.buffer.size);
...
Client.prototype.handleReceivedData = function (socket) {
    var smallBuf = socket.buffer.read(8, false); //ready 8 bytes, 64 bits
    var size = smallBuf.readUInt32BE(0) + 4; //why +4?
    var correlationId = smallBuf.readUInt32BE(4);

    if (verbose) {
        console.log("size", size);
        console.log("correlationId", correlationId);
        console.log("socket.buffer.size", socket.buffer.size);
    }

    if (socket.buffer.size >= size) {
        var resp = socket.buffer.read(size);
        if (verbose) console.log("resp.length", resp.length);
        ...
        if (verbose) console.log("cb socket.buffer.size", socket.buffer.size);
