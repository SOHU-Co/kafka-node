...
Client.prototype.sendOffsetFetchV1Request = function (group, payloads, cb) {
  var encoder = protocol.encodeOffsetFetchV1Request(group);
...
Client.prototype.sendGroupCoordinatorRequest = function (groupId, cb) {
  ...
  var request = protocol.encodeGroupCoordinatorRequest(this.clientId, correlationId, groupId);
  ...
  this.queueCallback(broker.socket, correlationId, [protocol.decodeGroupCoordinatorResponse, cb]);
  broker.write(request);
};

Client.prototype.sendJoinGroupRequest = function (groupId, memberId, sessionTimeout, groupProtocol, cb) {
  var correlationId = this.nextId();
  var request = protocol.encodeJoinGroupRequest(this.clientId, correlationId, groupId, memberId || '', sessionTimeout, groupProtocol);
  debug('JoinGroup Request', request.toString());
  ...
  this.queueCallback(broker.socket, correlationId, [protocol.decodeJoinGroupResponse, cb]);
  broker.write(request);
};

Client.prototype.sendSyncGroupRequest = function (groupId, generationId, memberId, groupAssignment, cb) {
  var correlationId = this.nextId();
  var request = protocol.encodeSyncGroupRequest(this.clientId, correlationId, groupId, generationId, memberId, groupAssignment);
  debug('groupAssignment', groupAssignment);
  debug('SyncGroup Request', request.toString());
  var broker = this.brokerForLeader(this.coordinatorId);

  if (!broker || !broker.socket || broker.socket.error) {
    return cb(new errors.BrokerNotAvailableError('Broker not available'));
  }
  this.queueCallback(broker.socket, correlationId, [protocol.decodeSyncGroupResponse, cb]);
  broker.write(request);
};

Client.prototype.sendHeartbeatRequest = function (groupId, generationId, memberId, cb) {
  var correlationId = this.nextId();
  var request = protocol.encodeGroupHeartbeat(this.clientId, correlationId, groupId, generationId, memberId);

  console.log('heatbeat request:', request.toString());
  var broker = this.brokerForLeader();
  ...
  this.queueCallback(broker.socket, correlationId, [protocol.decodeGroupHeartbeat, cb]);
  broker.write(request);
};
