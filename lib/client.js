...
Client.prototype.loadMetadataForTopics = function (topics, cb) {
    ...
    var cbb = function (err, resp) {
        if ((err) || (resp[1].error)) {
            if (self.retriesAttempts < self.options.retries) {
                self.retriesAttempts += 1;

                console.log(err || resp[1].error, ", retrying ...");

                setTimeout(function () {
                    self.loadMetadataForTopics(topics, cb)
                }, self.options.retriesBackoffMs)
                return;
            }
        }

        self.retriesAttempts = 0;
        cb(err, resp);
    }
