...
Client.prototype.sendProduceRequest = function (payloads, requireAcks, ackTimeoutMs, apiVersion, cb) {
  let encoder = protocol.encodeProduceRequest(requireAcks, ackTimeoutMs, apiVersion);
  ...
  switch (apiVersion) {
    ...
    case 1:
      decoder = protocol.decodeProduceV1Response;
      break;

    case 2:
      decoder = protocol.decodeProduceV2Response;
      break;
  }
  ...
  function buildRequest (payload, cb) {
    ...
    let codec = getCodec(attributes);

    if (!codec) return cb();

    let innerSet = encodeMessageSet(payload.messages);
    codec.encode(innerSet, function (err, message) {
      if (err) return cb(err);
      payload.messages = [ new Message(protocol.messageMagic(apiVersion), attributes, '', message, new Date().getTime()) ];
      cb();
    });
  }
};
