...
Client.prototype.send = function (payloads, encoder, decoder, cb) {
    ...
    if (payloads[0].length && !payloads[1].length) {
        this.sendToBroker(payloads, encoder, decoder, cb);
        return;
    }
...
Client.prototype.sendFetchRequest = function (payloads, fetchMaxWaitMs, fetchMinBytes) {
    var encoder = protocol.encodeFetchRequest(fetchMaxWaitMs, fetchMinBytes),
        decoder = protocol.decodeFetchResponse;
...
Client.prototype.updateMetadatas = function (metadatas) {
    this.brokerMetadata = 
        this.brokerMetadata ? _.extend(this.brokerMetadata, metadatas[0]) : metadatas[0];
...
Client.prototype.handleReceivedData = function (socket) {
    ...
    var size = Binary.parse(socket.buffer).word32bu('size').vars.size + 4;
    if (socket.buffer.length >= size) {
        var resp = new Buffer(size);
        socket.buffer.copy(resp, 0, 0, size);
        var correlationId = Binary.parse(resp)
                            .word32bu('size')
...
global.g.push(correlationId);
console.log('c:',global.c.length,'g:', global.g.length, 'm:', global.m.length)
        var data = decoder(resp);
        data.count = global.c.length;
        typeof cb === 'string' ?
            this.emit('message', decoder(resp)) :
            cb && cb.call(this, data);

        socket.buffer = buffer.slice(size);
