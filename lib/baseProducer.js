 ...
 *
 * @param {Number} [options.apiVersion=2] Api version used for producer
 *
 * @param {Number} [defaultPartitionerType] The default partitioner type
...
function BaseProducer (client, options, defaultPartitionerType, customPartitioner) {
  ...
  this.apiVersion = options.apiVersion === undefined
    ? DEFAULTS.apiVersion
    : options.apiVersion;
...
BaseProducer.prototype.send = function (payloads, cb) {
  ...
  client.sendProduceRequest(this.buildPayloads(payloads, client.topicMetadata, apiVersion), requireAcks, ackTimeoutMs, apiVersion, cb);
};

BaseProducer.prototype.buildPayloads = function (payloads, topicMetadata, apiVersion) {
  ...
  payloads.forEach((p) => {
    ...
    messages = messages.map(function (message) {
      ...
      if (message instanceof KeyedMessage) {
        ...
        output = new Message(protocol.messageMagic(apiVersion), 0, '', message, new Date().getTime());
      }
